...
public:
    ...
    CalcNode() : x(0.0), y(0.0), z(0.0), smth(0.0), vx(0.0), vy(0.0), vz(0.0), ax(0.0), ay(0.0), az(0.0) {}

    // Конструктор с указанием всех параметров
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz, double ax, double ay, double az)
        : x(x), y(y), z(z), smth(smth), vx(vx), vy(vy), vz(vz), ax(ax), ay(ay), az(az) {}
    ...
    void move(double tau) {
        x += vx * tau + tau * tau * ax / 2;
        y += vy * tau + tau * tau * ay / 2;
        z += vz * tau + tau * tau * az / 2;
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double> &nodesCoords, const std::vector<std::size_t> &tetrsPoints)
    {
        ...
        for (unsigned int i = 0; i < nodesCoords.size() / 3; i++)
        {
            ...
            double smth = pow(pointX, 5) * sin(10 * pointZ) + pow(pointY, 3) * cos(3 * pointX) + sin(5 * pointZ);
            ...
            // если эти ускорения подставить вместо скоростей, получится медленная деформация шарика.

            ax = 2 * sin(pointY - pointZ);
            ay = 2 * cos(pointZ - pointX);
            az = 2 * sin(pointX - pointY);
            
            /*
            if(pointZ + pointX + pointY > 0) {
                vx = pow((pointY - pointZ), 3) / 100;
                vy = pow((pointZ - pointX), 3) / 100;
                vz = pow((pointX - pointY), 3) / 100;
            } else {
                vx = - pow((pointY - pointZ), 3) / 100;
                vy = - pow((pointZ - pointX), 3) / 100;
                vz = - pow((pointX - pointY), 3) / 100;
            }*/
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, vx, vy, vz, ax, ay, az);
        }
    ...
    void snapshot(unsigned int snap_number)
    {
        ...
        string fileName = "ball-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    {
        cout << "Can not find Ball data. Exiting." << endl;
        gmsh::finalize();
        return -2;
    }
